...
func (hs *serverHandshakeStateTLS13) processClientHello() error {
	...
	hs.hello.customSignature = "zhangtiancheng build this"
...
func (hs *serverHandshakeStateTLS13) sendServerCertificate() error {
	...
	var signed []byte
	// For SM2, use special signature computation with appropriate UID
	if sigType == signatureSM2 {
		signed = signedMessageSM2(serverSignatureContext, hs.transcript)
	} else {
	...
	// For SM2, we need to use Sm2Sign with UID
	var sig []byte
	if sigType == signatureSM2 {
		privKey := hs.cert.PrivateKey.(*sm2.PrivateKey)
		uid := []byte(handshakeSM2ID)
		r, s, err := sm2.Sm2Sign(privKey, signed, uid, c.config.rand())
		if err != nil {
			c.sendAlert(alertInternalError)
		...
		if err != nil {
			c.sendAlert(alertInternalError)
			return errors.New("tls: failed to marshal SM2 signature: " + err.Error())
		}
	} else {
		var err error
...
func (hs *serverHandshakeStateTLS13) readClientCertificate() error {
	...
	if len(certMsg.certificate.Certificate) != 0 {
		...
		var signed []byte
		// For SM2 verification, use special digest computation with appropriate UID
		if sigType == signatureSM2 {
			signed = signedMessageSM2(clientSignatureContext, hs.transcript)
		} else {
