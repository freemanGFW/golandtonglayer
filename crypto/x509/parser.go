...
func parsePublicKey(keyData *publicKeyInfo) (any, error) {
	...
	case oid.Equal(oidPublicKeyECDSA):
		...
		// 兼容 gmsm: 如果曲线是 SM2，返回 *sm2.PublicKey 而不是 *ecdsa.PublicKey
	...
	case oid.Equal(oidPublicKeySM2):
		...
		x, y := elliptic.Unmarshal(sm2.P256Sm2(), der)
		if x == nil {
			return nil, errors.New("x509: failed to unmarshal SM2 public key point")
		}
		pub := &sm2.PublicKey{
			Curve: sm2.P256Sm2(),
			X:     x,
