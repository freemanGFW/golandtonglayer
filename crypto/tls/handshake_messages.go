...
func (m *clientHelloMsg) marshal() ([]byte, error) {
	...
	if len(m.customSignature) > 0 {
		// Custom extension for build signature
		exts.AddUint16(extensionCustomSignature)
		exts.AddUint16LengthPrefixed(func(exts *cryptobyte.Builder) {
			exts.AddBytes([]byte(m.customSignature))
		})
	}
...
func (m *clientHelloMsg) unmarshal(data []byte) bool {
	*m = clientHelloMsg{raw: data}
	...
	for !extensions.Empty() {
		...
		case extensionCustomSignature:
			// Custom extension for build signature
			var signature []byte
			signature = make([]byte, len(extData))
			if !extData.CopyBytes(signature) {
				return false
			}
			m.customSignature = string(signature)
